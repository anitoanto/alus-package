            ...
            if save_img:
                if dataset.mode == "image":
                    ...
                    if vid_path != save_path:  # new video
                        ...
                        fourcc = "VP80"  # output video codec
                        fps = vid_cap.get(cv2.CAP_PROP_FPS)
                        w = int(vid_cap.get(cv2.CAP_PROP_FRAME_WIDTH))
                        h = int(vid_cap.get(cv2.CAP_PROP_FRAME_HEIGHT))
                        vid_writer = cv2.VideoWriter(
                            save_path + ".normal_objTag.webm", cv2.VideoWriter_fourcc(*fourcc), fps, (w, h)
                        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--weights", nargs="+", type=str, default=mdl_weight, help="model.pt path(s)"
    )
    parser.add_argument(
        "--source", type=str, default=dirpath, help="source"
